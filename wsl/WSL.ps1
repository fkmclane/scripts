...
Start-Process wsl '-e',bash,-c,'"cd ~; export SHELL=`getent passwd | awk -F: -v user=\"$USER\" ''$1 == user {print $NF}''`; export DISPLAY=`ip route list default | head -n1 | sed -e ''s/^default via \([0-9a-f.:]\+\) .*$/\1/''`:0; exec /usr/bin/x-terminal-emulator -e \"$SHELL\""' -NoNewWindow
